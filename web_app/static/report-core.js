(function () {
  ...
  function renderTable(container, data, options = {}) {
    ...
    data.columns.forEach((col) => {
      ...
      if (mode === "default" && resourceColumnsPresent(data.columns)) {
        const aggregateColumnVisible = data.columns.includes(RESOURCE_AGGREGATE_COLUMN);
        const shouldShowToggle = (aggregateColumnVisible && String(col) === RESOURCE_AGGREGATE_COLUMN) || (!aggregateColumnVisible && String(col) === "ФОТ");
        if (shouldShowToggle && typeof options.onResourceToggle === "function") {
